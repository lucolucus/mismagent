...
`/skill:mismagent-model`; the single command is not exempt (friction-log-4 #14).

**The FOUNDATIONAL dispatch — two-pass (the agent is headless).** Dispatch the
**`mismagent-architect`** subagent (the `subagent` tool) for `$ARGUMENTS` (or the current feature) with
