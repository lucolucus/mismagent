...
flowchart TD
    ...
    subgraph EX["explore — you in dialogue"]
        ...
        chal["mism-challenger — subagent, fresh context<br/>KILL · RESHAPE · PROCEED"]
        res["mism-researcher — subagent<br/>explores the domain, gathers material (if new)"]
        ana["mism-analyst — subagent<br/>models the strategic + ubiquitous language"]
    ...
    subgraph MO["model — you confirm the boundaries"]
        direction TB
        tact["mism-tactical-modeler — subagent<br/>aggregates/invariants/events/commands → context-map"]
        ux["mism-ux-designer — skill<br/>imagines the UI → views (if there is UI)"]
        arch["mism-architect — subagent<br/>architecture + ADRs<br/>stack DELIBERATED with the user, then finalizes the gate"]
        bman["mism-build-manifest — skill<br/>tactical → building-block manifest<br/>types PINNED at the boundaries + tests_nl"]
        manifest[("building-blocks.yaml<br/>blocks + boundaries + projection")]
        ccon["mism-create-contract — skill, cross-deploy MODULE<br/>ONLY cross-deploy boundaries → OpenAPI"]
    ...
    subgraph BU["build — you delegate; confirm only at the end"]
        direction TB
        comp["/mismagent:composer — command<br/>owner-first waves · D1 green on its own ·<br/>merge=composition · D2 boundary welded"]
        wrk["mism-worker ×N — subagent<br/>skill = block-type × projection + side memory"]
        ver["mism-verifier (structural) + mism-code-review (semantic)"]
...
- skill **`/mismagent:mism-explore`** (you dialogue in session; **profile bootstrap** if missing:
  `output_dir` default `.mismagent` + language of the names) · subagent **`mism-researcher`**
  (explores the domain → `research/<topic>.md`, when the domain is new) · subagent
  **`mism-challenger`** (with fresh context tries to *demolish* the idea) · subagent
  **`mism-analyst`** (models the **strategic**: bounded contexts + **ubiquitous language** in the
...
1. **`mism-tactical-modeler`** `[subagent]` — completes the model: aggregates/invariants/events/
   commands per context (it starts from the context-map's "Seeds for the tactical").
2. **`/mismagent:mism-ux-designer`** `[skill]` — imagines the UI → views (only if there is UI).
3. **`mism-architect`** `[subagent]` — architecture + ADRs + boundaries with projection.
   **Foundational decisions (stack/language) are deliberated WITH the user** (alternatives +
   ...
   `gate` in the profile**.
4. **`/mismagent:mism-build-manifest`** `[skill]` — the tactical → `building-blocks.yaml`:
   blocks + boundaries with **PINNED types** (Published Language) + projection + the user's `tests_nl`.
5. **`/mismagent-cross-deploy:mism-create-contract`** `[skill, from the cross-deploy module]` —
...
- command **`/mismagent:composer <feature>`** — thin coordinator, the only one that merges and
  moves state: readiness on the manifest (pinned types, or BOUNCE to IDEA-2) →
  *boundary-owner-first* waves → dispatches **`mism-worker`** ×N `[subagent]` (skill = block-type ×
  projection + the side's dev-architecture + tier) → **D1** green on its own (fresh `mism-verifier` +
  `mism-code-review`) → merge = composition → **D2** contract test on the welded boundary →
...
Verify: `/mismagent:mism-explore` appears among the skills.

**1 · explore — you in dialogue (high presence).**
You type **`/mismagent:mism-explore <the idea in one sentence>`**. The skill: (step 0) if missing,
creates the bootstrap `.mismagent/profile.md` (output_dir, language of the names, sides); dialogues
with you; dispatches **`mism-challenger`** (KILL → stop · RESHAPE → redesign with you ·
PROCEED → go on), if needed **`mism-researcher`**, then **`mism-analyst`** (context-map +
...
1. You say: *"dispatch `mism-tactical-modeler`"* → Tactical model in the context-map (it absorbs
   the Seeds); on `NEEDS-INPUT` it brings you the ambiguities, you decide.
2. *(if there is UI)* you type **`/mismagent:mism-ux-designer`** → concept with you → `UI/ux-proposal.md`.
3. You say: *"dispatch `mism-architect`"* → it presents the **stack alternatives with pros/cons
   and YOU choose** (never a silent ADR) → ADRs + boundaries with projection → it finalizes the
   `gate` in the profile.
4. You type **`/mismagent:mism-build-manifest`** → `building-blocks.yaml` (types PINNED at the
...
5. *(only if a boundary is cross-deploy)* you type
   **`/mismagent-cross-deploy:mism-create-contract`** → ONE OpenAPI.
*Gate:* `/mismagent:mism-readiness-gate` (or directly Phase 1 of the Composer). → build.

**3 · build — you delegate; confirm only at the end.**
Prerequisite: the side's repo is **under git** (the Composer lives on worktrees and merges).
You type **`/mismagent:composer <feature>`**. It: readiness (unpinned boundary →
BOUNCE to IDEA-2) → owner-first waves → dispatches **`mism-worker`** ×N → D1 (verifier +
...
1. **state = the folder** (`todo/ doing/ done/`); `git mv` and merges only by the Composer.
