...
`DECISIONS`, appended with `python3 "${CLAUDE_PLUGIN_ROOT}/tools/mismagent.py" why append <file> --entry <entry-file>`
(format: `${CLAUDE_PLUGIN_ROOT}/tools/CLI.md`, its short form when a requirement or the scope
...
4. **Manifest** *(feature: no `building-blocks.yaml`, or the model changed)* — the `build-manifest`
   ...
   `/mismagent:board`. It closes with its blocking `lint`: zero gaps.
