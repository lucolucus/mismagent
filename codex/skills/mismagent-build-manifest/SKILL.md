...
11. **The invariant tag is a verifiable convention:** each invariant test's name carries its tag —
    canonical `INV-n` in the docs, spelled as the dev-architecture fixes for the stack (e.g. `INV_12`),
...
14. **Shared artifact ⇒ derived owner block** in an earlier wave, computed from the manifest:
    shared-kernel domain types (VOs, enums, their invariants — the Shared Kernel section) → an
    ordinary, reviewed owner block; a technical artifact ≥ 2 blocks of a wave consume (a module's
    build files, a ui-kit, a schema/migration set, DI wiring) → one owner block, no domain rules.
...
16. **Group infrastructure by module:** the adapters of one infrastructure module are one block that
    ...
    in different waves or different releases. Domain blocks keep one aggregate = one block.
...
block files — no other task list (the human reads them with `$mismagent-board`).
