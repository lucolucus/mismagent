...
- **Trunk absent** (`architecture.md` or `code-rules.md` missing, or `gate` still
  ...
  `@@MISMAGENT_SKILLS@@/mismagent-worker-composer/references/CLI.md`), then re-dispatch pass 2 to write the trunk and
...
An artifact that already exists is stated, not re-deliberated (same rule as `/skill:mismagent-model`).
