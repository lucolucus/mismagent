...
The human reads the work via **`/skill:mismagent-board [feature]`**: a read-only server that scans the block
...
`/skill:mismagent-worker-composer`. Tell the user the block files are seeded in
`blocks/<context>/todo/` and that **`/skill:mismagent-board`** shows them live.
