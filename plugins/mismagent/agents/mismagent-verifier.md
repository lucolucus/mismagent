...
3. **AC coverage (you own it):** every `## Tasks` criterion has a test in the diff, else FAIL
   listing them. A cross-deploy operation needs its shape test per `contract_form` (`openapi`: the
   real response body of its `operationId`; `event-schema`: the generated event types). An AC
   claiming **concurrency** needs a test that visibly creates contention (N threads/coroutines + a
   start barrier); a sequential test leaves it uncovered.
4. **Contract referenced, not duplicated:** cross-deploy types come from the contract-generated
   types; in-process ports and shared-kernel values are imported, never re-declared → else FAIL.
5. **No shadow types:** a domain type the diff introduces that renames a canonical term (the
   context-map's language) or hand-copies a generated type → FAIL.
...
7. **Invariants and errors (a block exposing a write):** each invariant AC has a test; the failure
   side too — cross-deploy: the error responses the contract declares; in-process: the command's
   rejection criteria. Missing → FAIL.
