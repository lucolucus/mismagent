...
`HEAD_SHA` from `MM diff-range`, and the block's `MM pack` (`MM` = `python3 "@@MISMAGENT_SKILLS@@/mismagent-worker-composer/scripts/mismagent.py"`). Read-only → `PASS | FAIL | SKIP`; an objection to a `D-NNNN` goes into that entry's `Debate`
(`F/decisions.md`). See
